define(['./prsly', './tokens', './models/ddm'], function(_, T, DDM) {
  ...
  function to_tree_of_values(tree_of_blocks) {
    ...
    if (is_listy(tree_of_values))
      target_children = tree_of_values.items[tree_of_values.items.length - 1].children
    else
      target_children = tree_of_values.children
    
    target_children.push(...tree_of_blocks.children.map(to_tree_of_values))
